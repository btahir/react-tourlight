...
The core library is **~5KB gzipped** with no bundled runtime dependencies of its own — `@floating-ui/react-dom` is required, but as a peer dependency, so it's installed once and shared instead of duplicated. Floating UI itself adds roughly ~4KB gzipped.
