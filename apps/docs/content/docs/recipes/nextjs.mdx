...
description: Set up react-spotlight with Next.js App Router, including client component wrappers and SSR considerations.
---

react-spotlight is a client-side library that uses browser APIs (`MutationObserver`, `document.querySelector`, `inert`). In Next.js App Router, you need to mark the provider as a Client Component.
...
react-spotlight renders nothing on the server. The overlay, tooltip, and spotlight cutout are all client-side only. This means:
