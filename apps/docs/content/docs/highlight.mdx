...
## Opt-in callouts with a beacon

To let users discover a callout themselves, anchor a [beacon](/docs/beacons)
to the element and pass it a `highlight`:

```tsx
<SpotlightBeacon
  target="#export-button"
  highlight={{ title: 'New: CSV Export', content: 'You can now export your data as CSV.' }}
/>
```
