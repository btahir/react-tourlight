...
<SpotlightTour
  id="onboarding"
  steps={steps}
  renderTooltip={({ step, next, previous, skip, close, currentIndex, totalSteps, isFirst, isLast }) => (
    <div className="my-tooltip">
      ...
      <footer>
        ...
        {!isLast && <button onClick={skip}>Skip</button>}
...
| `skip` | `() => void` | Skip the tour (fires `onSkip`) |
| `close` | `() => void` | Stop the tour without marking it completed or skipped (the "×" action) |
