...
react-spotlight ships with `light` and `dark` themes, plus an `auto` mode that follows the user's OS preference via `prefers-color-scheme`.
...
```tsx
import { darkTheme } from 'react-spotlight'
